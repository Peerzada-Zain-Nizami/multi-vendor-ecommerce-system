@php
    ...
    class categories{
        ...
        {
            $data = SellerCategory::where('parent_id',$id)->with('children')->get();
            foreach ($data as $category)
            {
                echo '<li><a href="javascript:void(0);" id="'.$category->id.'" class="cat_click" name="'.\App\MyClasses\Helpers::get_lang($category->category_name,$category->id,"seller_category",App::getLocale()).'">'.\App\MyClasses\Helpers::get_lang($category->category_name,$category->id,"seller_category",App::getLocale()).'</a>';
                ...
                <div class="col-sm-3">
                    <div class="card">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="parent_category" class="form-control select2-show-search">
                                        ...
                                        @foreach($results as $result)
                                            <option value="{{$result->id}}">{{\App\MyClasses\Helpers::get_lang($result->category_name,$result->id,"seller_category",App::getLocale())}}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div id="form" style="display: none;">
                            ...
                            <div class="card-body">
                                <button class="btn btn-primary btn-sm add_lang_model">{{__('messages.add-language')}}</button>
                                <table class="table">
                                    <thead>
                                    <tr>
                            ...
                            <div class="card-body">
                                ...
                                <form>
                                    ...
                                    <div class="col-12">
                                        <button class="btn btn-primary add_sub" type="button">{{__('messages.add-sub-category')}}</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            <ul id="tree1">
                                @foreach($categories as $category)
                                    <li><a href="javascript:void(0);" id="{{$category->id}}" class="cat_click" name="{{\App\MyClasses\Helpers::get_lang($category->category_name,$category->id,"seller_category",App::getLocale())}}">{{\App\MyClasses\Helpers::get_lang($category->category_name,$category->id,"seller_category",App::getLocale())}}</a>
                                        @if ($category->children->isNotEmpty())
                                            <ul>
...
@section('query')
    <script type="application/javascript">
        $(document).on('click','.add_lang_model',function () {
            ...
            var data = {
                'ref_id': id,
                'ref_type': "seller_category",
            };
            $.ajax({
                url: "{{route('seller.get.languages')}}",
                type:"POST",
                dataType: "json",
                data: data,
        ...
        $(document).on('click','.update_lang_model',function () {
            var id = $(this).attr('id');
            console.log(id);
            var route = "{{route('seller.category.lang.add')}}";
        ...
        $(Document).ready(function() {
            ...
            $('.cat_click').click(function () {
                ...
                $.ajax({
                    url: "{{route('seller.get.languages_2')}}",
                    type: "POST",
                    dataType: "json",
                    data: data,
                    success: function (response) {
          ...
          $('.del').click(function () {
    var id = $('#id').val(); // Fetch the ID from input
    swal({
        title: "Are you sure?",
        ...
        if (willDelete) {
            $.ajax({
                ...
                success: function (data) {
                    if (data.status === 'success') {
                        ...
                        Swal.fire({
                            title: 'Error!',
                            text: data.message, // Display error message returned from the server
                            icon: 'error',
                            showConfirmButton: true,
                        });
                    }
            ...
            $('.update').click(function(){
                ...
                $.ajax({
                    url:'{{route('seller.category.update')}}',
                    type:'POST',
                    data:update,
                    success:function(response){
            ...
            $('.add_lang').click(function () {
                ...
                console.log(data);
                $.ajax({
                    url: "{{route('seller.category.lang.add')}}",
                    type:"POST",
                    dataType: "json",
                    data: data,
                    success: function(response){
            ...
            $('.update_lang').click(function () {
                ...
                console.log(data);
                $.ajax({
                    url: "{{route('seller.category.lang.update')}}",
                    type:"POST",
                    dataType: "json",
                    data: data,
                    success: function(response){
            ...
            function loct() {
                window.location.href = '{{route('seller.category.list')}}';
            }
