                    ...
                    <table id="example" class="table table-responsive-lg-sm table-hover table-bordered text-nowrap key-buttons">
                        ...
                        @foreach($results as $result)
                            ...
                            <tr>
                                ...
                                <td><a href="{{route('admin.view.invoice',['id'=> $result->id])}}">{{$result->invoice_no}}</a></td>
                                ...
                                <td>
                                    <div class="btn-group">
                                        <a href="javascript:void(0);" class="btn btn-light btn-sm" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{__('messages.action')}} <i class="fa fa-angle-down"></i></a>
                                        <div class="dropdown-menu" style="">
                                            <a class="dropdown-item" href="{{route('admin.view.invoice.return',['id'=> $result->id])}}"><i class="fa fa-eye me-2"></i> {{__('messages.view')}}</a>
                                            @if($result->status == "Complete" && $result->payment != "Paid" && $result->payment != "Return")
                                                <a class="dropdown-item" href="javascript:void(0);" onclick="model({{$result->id}},{{$result->remaining}})"><i class="fa fa-dollar me-2"></i> {{__('messages.receive')}}</a>
                                            @endif
                                        </div>
                                    </div>
                                </td>
            ...
            <div class="modal-content modal-content-demo">
                <form method="POST" id="pay_form" enctype="multipart/form-data">
                    ...
                    <div class="modal-header">
                        <h6 class="modal-title">{{__('messages.receive-invoice-payment')}}</h6><button aria-label="Close" class="btn-close" data-bs-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                    </div>
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-primary" type="submit">{{__('messages.receive')}}</button> <button class="btn btn-secondary" data-bs-dismiss="modal" type="button">{{__('messages.close')}}</button>
                    </div>
                </form>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('#pay_form').submit(function (e) {
                ...
                $.ajax({
                    url: "{{route('admin.invoice.return.pay')}}",
                    method:"POST",
                    contentType: false,
                    processData: false,
                    data: formData,
                    success: function(response){
                        if (response.status == 400)
                        {
                            ...
                            setTimeout(function() {
                                window.location.href = '{{route('admin.manage.invoice.return')}}';
                            }, 2000);
                        }
                    }
                })
