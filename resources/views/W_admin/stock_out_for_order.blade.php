        ...
        <div class="side-app">
            ...
            <div class="page-header">
                <div class="page-leftheader">
                    <h4 class="page-title mb-0 text-primary">{{__('messages.stock-out')}}</h4>
            ...
            <div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <input type="text" id="product_barcode" list="shelf_code" onchange="selected_item({{$shelf->id}})" class="form-control" autofocus>
                                <input type="hidden" id="order_id" value="{{$order_id}}" autofocus>
                                <input type="hidden" id="shelf_id" value="{{$shelf->id}}" autofocus>
                                <input type="hidden" id="stock_ins" value="???" autofocus>
                                <input type="hidden" id="stockins_list_id" value="???" autofocus>
                                <datalist id="shelf_code">
                                    @foreach($stocks as $stock)
                                        <option value="{{$stock['stock']->stock_ins_id}}">{{$stock['stock']->stock_ins_id}}</option>
                                    @endforeach
                                </datalist>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-1">
                            ...
                            <label class="custom-switch">
                                <input value="{{$user->id}}" id="stock_out_check" type="checkbox" class="custom-switch-input on_off" @if($user->stock_out_check == "true")checked @endif>
                        ...
                        <label>Action</label>
                        <br>
                        <button id="stock_out" class="btn btn-warning">{{__('messages.stock-out')}}</button>
                    </div>
                    </div>
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">{{__('messages.stock-out')}}</div>
                    </div>
                    <div class="card-body">
                ...
                <div class="text-center">
                    <form action="{{route('wadmin.stockOut.place.stock.update.for.order',$shelf->id)}}" method="post">
                        @csrf
...
@section('query')
    <script type="text/javascript">
        ...
        function selected_item(shelf_id)
        {
            ...
            console.log(data);
            $.ajax({
                url:'{{route('wadmin.stock.out.get.stock.length.for.order')}}',
                type:'POST',
                data:data,
                success:function(response){
                    if (response.status == "fail")
                    {
                        ...
                        setInterval(function () {
                            $('#error').removeClass('alert alert-light-success');
                            $('#error').html('');
                        },8000);
                    }
                    else {
                        ...
                        $("#stock_ins").attr({
                            "value" : response.data.stock_ins_id,
                        });
        ...
        {
            $('#loader').show();
            var route = "{{route('wadmin.stockOut.place.data')}}";
            $.get(route,function (data) {
                console.log(data);
                if(data.stock_out.length == 0)
                {
        ...
        function auto_sync()
        {
            ...
            $.ajax({
                url:'{{route('wadmin.stock.place.stock_out.for.order')}}',
                type:'POST',
                data:data,
                success:function(response){
                    if (response.status == "fail")
        ...
        $('.on_off').click(function () {
            ...
            if ($(this).is(':checked') == true)
            {
                ...
                $('#stock_out').attr('disabled','disabled');
        ...
        $('#stock_out').click(function(){
            ...
            console.log(data);
            $.ajax({
                url:'{{route('wadmin.stock.place.stock_out_for_order')}}',
                type:'POST',
                data:data,
                success:function(response){
                    if (response.status == "fail")
        ...
        $(document).on('click','.minus',function () {
            var id = $(this).attr("id");
            $.ajax({
                url:'{{route('wadmin.stockOut.place.stock.minus.for.order')}}',
                type:'post',
                data:{id:id},
        ...
        $(document).on('click','.plus',function () {
            ...
            console.log(data);
            $.ajax({
                url:'{{route('wadmin.stockOut.place.stock.plus.for.order')}}',
                type:'post',
                data:data,
                success:function(data){
                    console.log(data);
        ...
        $(document).on('click','.del',function(){
            ...
            swal({
                ...
                .then((willDelete) => {
                    ...
                    if (willDelete) {
                        $.ajax({
                            url:'{{route('wadmin.stockOut.place.stock.del.for.order')}}',
                            type:'POST',
                            data:{id:id},
                            success:function(data){
                                load_data();
        ...
        scanner.addListener('scan', function (content) {
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    console.log(response.data.stock_ins_id);
                    $("#stock_ins").attr({
                        "value" : response.data.stock_ins_id,
                    });
                    $("#stockins_list_id").attr({
