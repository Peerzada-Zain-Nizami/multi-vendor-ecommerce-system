        ...
        <div class="side-app">
            ...
            <div class="page-header">
                <div class="page-leftheader">
                    <h4 class="page-title mb-0 text-primary">{{__('messages.stock-move')}}</h4>
            ...
            <div>
                ...
                <div id="success">
                    ...
                    <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            @php
                                /*dd($stocks[0]['stock']->stock_ins_id);*/
                            @endphp
                        ...
                        <div class="col-sm-1">
                            ...
                            <label class="custom-switch">
                                <input value="{{$user->id}}" id="stock_move_check" type="checkbox" class="custom-switch-input on_off" @if($user->stock_move_check == "true")checked @endif>
                        ...
                        <div class="col-sm-2">
                            ...
                            <br>
                            <button id="stock_move" class="btn btn-primary">{{__('messages.stock-move')}}</button>
                        </div>
                    </div>
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">{{__('messages.stock-move')}}</div>
                    </div>
                    <div class="card-body">
                ...
                <div class="text-center">
                    <form action="{{route('wadmin.stockMove.place.stock.move',$shelf->id)}}" method="get">
                        @csrf
                        <input id="update_btn" type="submit" class="btn btn-primary" value="{{__('messages.move-to')}}" disabled>
                    </form>
                </div>
            </div>
        </div>
...
@section('query')
    <script type="text/javascript">
        ...
        function selected_item(shelf_id)
        {
            ...
            console.log(data);
            $.ajax({
                url:'{{route('wadmin.stock.move.get.stock.length')}}',
                type:'POST',
                data:data,
        ...
        {
            $('#loader').show();
            var route = "{{route('wadmin.stockMove.place.data')}}";
            $.get(route,function (data) {
                console.log(data);
                if(data.stock_move.length == 0)
                {
        ...
        function auto_sync()
        {
            ...
            $.ajax({
                url:'{{route('wadmin.stock.place.stock_move')}}',
                type:'POST',
                data:data,
                success:function(response){
        ...
        $('.on_off').click(function () {
            ...
            if ($(this).is(':checked') == true)
            {
                ...
                $('#stock_move').attr('disabled','disabled');
                ...
                $('#stock_move').removeAttr('disabled');
                $('#quantity').removeAttr('disabled');
                $('#quantity').val('0');
            }
        });
        $('#stock_move').click(function(){
            var pro_id = $('#product_barcode').val();
            var shelf_id = $('#shelf_id').val();
            var stock_ins = $('#stock_ins').val();
            var stockins_list_id = $('#stockins_list_id').val();
        ...
        $(document).on('click','.minus',function () {
            var id = $(this).attr("id");
            $.ajax({
                url:'{{route('wadmin.stockMove.place.stock.minus')}}',
                type:'post',
                data:{id:id},
        ...
        $(document).on('click','.plus',function () {
            var id = $(this).attr("id");
            $.ajax({
                url:'{{route('wadmin.stockMove.place.stock.plus')}}',
                type:'post',
                data:{id:id},
                success:function(data){
                    if (data.status == "fail")
        ...
        $(document).on('click','.del',function(){
            ...
            swal({
                ...
                .then((willDelete) => {
                    ...
                    if (willDelete) {
                        $.ajax({
                            url:'{{route('wadmin.stockMove.place.stock.del')}}',
                            type:'POST',
                            data:{id:id},
                            success:function(data){
                                load_data();
