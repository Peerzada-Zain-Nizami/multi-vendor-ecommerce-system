                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive-lg">
                                        ...
                                        @if (!empty($ok->AWB_no))
                                            <tr>
                                                ...
                                                <td class="text-end"><span class="font-weight-bold  ms-auto text-info">
                                                        @if ($datas->order_status == 'CANCELLED ON CLIENTS REQUEST')
                                ...
                                @endif
                                {{-- end Changing status before order --}}

                                {{-- Out from stock --}}
                                {{-- @if (isset($flag) == true)
                                    @if ($datas->status == 'Processing' && !in_array($datas->refund_status, ['return-requested', 'return-approved', 'return-cancelled']))
                                        <div class="form-group text-center">
                                            <a href="{{Route('wadmin.order.product.place.tracking.page.out.for.stock',$datas->id)}}" class="btn btn-primary" @if (!empty($check))
                                                hidden @else @endif>{{__('messages.out-from-stock')}}</a>
                                        </div>
                                    @endif
                                @endif --}}
                                    ...
                                    <div class="btn-group mt-2 mb-2" style="left: 35%">
                                        ...
                                        <ul class="dropdown-menu" role="menu">
                                            <li><a id="mySelect" class="modal-effect" data-bs-effect="effect-scale"
                                                    ...
                                                    data-bs-target="#shelf_scanner_modal">By Camera</a></li>
...
<script type="text/javascript">
    ...
    $('#mySelect').on('click', function() {
        $('#camera_list1').html('');
        shelfScanner = new Instascan.Scanner({
            video: document.getElementById('preview1'),
            mirror: false
        });
        shelfScanner.addListener('scan', function(content) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        if (response.message !== 'Your Product has not Matched.') {
                            setTimeout(() => {
                                openProductscannerModal(data);
                            }, 2000);
                        }
        ...
        $('#shelf_scanner_modal').modal('hide'); // Close the shelf scanner modal
        $('#modaldemo8').modal('show'); // Open the product scanner modal
    })


    function openProductscannerModal(data) {
        $('#camera_list').html('');
