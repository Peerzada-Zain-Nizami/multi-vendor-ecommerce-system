                            ...
                            <table id="example" class="table table-responsive-lg-sm table-bordered text-nowrap key-buttons dataTable no-footer" role="grid" aria-describedby="example1_info">
                                ...
                                @foreach($warehouses as $warehouse)
                                    <tr>
                                        ...
                                        <td class="@if($warehouse->status == "Active") text-success @else text-danger @endif">@if($warehouse->status == "Active") {{__('messages.active')}} @else {{__('messages.deactivate')}} @endif</td>
                                        <td><a href="{{route('admin.warehouse.view',$warehouse->id)}}" class="btn btn-primary"><i class="fa fa-eye"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
